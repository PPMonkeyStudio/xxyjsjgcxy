...
function initialization() {
	...
	if (span_option.innerHTML == 'update') {
		...
		getAdminUUID();
		System.out.println(getAdminUUID());
	}
...
function create_editor() {
	 ...
	 * getNewsUUID在这里给接口
	 */
	var span_jsj_sadmin_admin_id = document
			.getElementById("span_jsj_sadmin_admin_id");
...
function getAdminUUID() {
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				var span_jsj_sadmin_admin_id = document
						.getElementById("span_jsj_sadmin_admin_id");
				span_jsj_sadmin_admin_id.innerHTML = xhr.responseText;
				/*
	...
	xhr.open("POST", "/xxyjsjgcxy/sadmin/admin_getAdminUUID");

	xhr.send(null);
}

function getAdminGmtCreate() {
	var dtCur = new Date();
